...
function renderAdmin(){
 ...
 if(page==="prod"){
  ...
  M.innerHTML=`<div class="ph"><div><h2>Lista di produzione</h2><p>Quanto cucinare di ogni piatto, sommando gli ordini. Alla chiusura degli ordini la lista non cambia più.</p></div><button class="btn sm sec" id="printProd">Stampa la lista</button></div>
  ...
  <h3 style="margin:24px 0 10px;font-size:18px">Ritiri e consegne, ora per ora</h3>
 ...
 if(page==="orders"){
  ...
  M.innerHTML=`<div class="ph"><div><h2>Ordini</h2><p>Conferma uno per uno o tutti insieme dopo la chiusura. A ogni passaggio il cliente riceve un WhatsApp.</p></div><div style="display:flex;gap:8px"><button class="btn sm" id="confirmAll" ${os.some(o=>o.st==="new")?"":"disabled"}>Conferma tutti</button><button class="btn sm sec" id="resetDemo">Ripristina esempi</button></div></div>
  ...
  bindDayTabs(M);
  M.querySelectorAll("[data-st]").forEach(b=>b.onclick=()=>{const o=orders.find(x=>x.n==b.dataset.n);const s=b.dataset.st;if(s==="rej"){if(!confirm(`Rifiutare l'ordine #${o.n}? Il cliente riceve un messaggio e, se ha già pagato, gli restituisci i soldi.`))return;o.st="rej";toast(`#${o.n} rifiutato, cliente avvisato`)}else{o.st=s;if(s==="done")o.closed=nowHM();toast({prep:`#${o.n} confermato, va in produzione`,ready:`#${o.n} pronto, cliente avvisato`,done:`#${o.n} chiuso`}[s])}persist();renderAdmin()});
 ...
 /* ---- Menù e giorni ---- */
 if(page==="menu"){
  M.innerHTML=`<div class="ph"><div><h2>Menù e giorni</h2><p>Per ogni piatto: giorni in cui si fa, porzioni massime, esaurito o nascosto. Le modifiche vanno subito nel negozio.</p></div><button class="btn sm" id="addProd">+ Nuovo piatto</button></div>
  ...
  M.querySelectorAll("[data-limit]").forEach(i=>i.onchange=()=>{const p=products.find(x=>x.id==i.dataset.limit);p.limit=i.value===""?null:Math.max(0,+i.value);persist();toast(p.limit==null?"Nessun tetto: si cucina quanto viene ordinato":`Tetto: ${p.limit} porzioni al giorno`)});
 ...
 if(page==="catering"){
  ...
  M.innerHTML=`<div class="ph"><div><h2>Richieste catering</h2><p>Arrivano dal modulo del negozio. Il preventivo va mandato entro 24 ore, con il link per l'acconto del 30%.</p></div></div>
 ...
 if(page==="stats"){
  ...
  M.innerHTML=`<div class="ph"><div><h2>Incassi</h2><p>Solo ordini online. Quello che vendi al banco resta sul registratore di cassa.</p></div></div>
  <div class="kpis"><div class="kpi"><div class="eyebrow">Incassato (consegnati e ritirati)</div><div class="v num">${eur(inc)}</div></div><div class="kpi"><div class="eyebrow">Ordini da chiudere</div><div class="v num">${eur(booked)}<small>prossimi giorni</small></div></div><div class="kpi"><div class="eyebrow">Scontrino medio</div><div class="v num">${eur(all.length?all.reduce((s,o)=>s+o.tot.tot,0)/all.length:0)}</div></div><div class="kpi warn"><div class="eyebrow">Contanti da incassare</div><div class="v num">${eur(cash)}</div></div></div>
 ...
 /* ---- Impostazioni ---- */
 if(page==="settings"){
  M.innerHTML=`<div class="ph"><div><h2>Impostazioni</h2><p>Chiusura degli ordini, tempi, orari, pagamenti e zone di consegna. Le modifiche vanno subito nel negozio.</p></div></div>
  <div class="grid2">
   ...
   <div class="card"><h3>Orari di apertura</h3><div class="hours">${Object.keys(DN).map(k=>{const h=settings.hours[k];return`<div class="row"><b>${DN[k]}</b>${h?`<span class="num">${h[0].join("–")}</span><span class="num">${h[1]?h[1].join("–"):""}</span>`:`<span class="muted">Chiuso</span><span></span>`}</div>`}).join("")}</div><p class="hint" style="margin:12px 0 0">In questa versione gli orari non si cambiano. Nel sito finito si cambiano da qui, insieme a ferie e chiusure straordinarie.</p>
  ...
  M.querySelectorAll("[data-pay]").forEach(l=>l.onclick=()=>{settings.pay[l.dataset.pay]=!settings.pay[l.dataset.pay];if(!Object.values(settings.pay).some(Boolean)){settings.pay[l.dataset.pay]=true;toast("Almeno un pagamento deve restare attivo");}persist();renderAdmin()});
