...
$("#pauseSw").onclick=()=>{settings.pause=!settings.pause;persist();renderAdmin();toast(settings.pause?"Prenotazioni online sospese":"Prenotazioni online riaperte")};
...
function renderAdmin(){
 ...
 if(page==="prod"){
  ...
  M.innerHTML=`<div class="ph"><div><h2>Lista di produzione</h2><p>Quanto cucinare per ogni giorno: è la somma delle prenotazioni. Quando le prenotazioni chiudono, la lista si congela e la cucina parte da qui.</p></div><button class="btn sm sec" id="printProd">Stampa la lista</button></div>
  ${dayTabs(k=>dayOrders(k).length)}
  <div class="kpis"><div class="kpi ${frozen?"":"warn"}"><div class="eyebrow">Prenotazioni per ${dlabel(aday).toLowerCase()}</div><div class="v">${frozen?"Chiuse":"Aperte"}<small>${st.why}</small></div></div><div class="kpi"><div class="eyebrow">Ordini</div><div class="v num">${os.length}<small>${os.filter(o=>o.mode==="consegna").length} consegne</small></div></div><div class="kpi"><div class="eyebrow">Porzioni da cucinare</div><div class="v num">${portions}</div></div><div class="kpi"><div class="eyebrow">Incasso previsto</div><div class="v num">${eur(rev)}<small>${eur(cash)} in contanti</small></div></div></div>
  <h3 style="margin:0 0 10px;font-size:18px">Produzione di ${dfull(aday)}</h3>
  <div class="tw"><table><tr><th>Piatto</th><th>Categoria</th><th style="width:140px">Porzioni prenotate</th><th>Tetto del giorno</th><th style="width:200px">Riempimento</th></tr>
  ${rows.length?rows.map(({p,q})=>{const lim=p.limit;const pct=lim?Math.min(100,Math.round(q/lim*100)):0;const cls=lim&&q>=lim?"c":lim&&pct>=80?"w":"";return`<tr><td><b>${esc(p.n)}</b></td><td class="small muted">${p.cat}</td><td class="num" style="font-weight:800;font-size:16px">${q}</td><td class="num">${lim?`${lim} <span class="small muted">· restano ${Math.max(0,lim-q)}</span>`:'<span class="muted">nessuno</span>'}</td><td>${lim?`<div class="bar"><i class="${cls}" style="width:${pct}%"></i></div>`:'<span class="small muted">su richiesta</span>'}</td></tr>`}).join("")+`<tr class="tot"><td colspan="2">Totale porzioni</td><td class="num">${portions}</td><td colspan="2"></td></tr>`:`<tr><td colspan="5" class="muted">Nessuna prenotazione per ${dlabel(aday).toLowerCase()}.</td></tr>`}</table></div>
  <h3 style="margin:24px 0 10px;font-size:18px">Chi ritira e quando</h3>
  <div class="tw"><table><tr><th>Ora</th><th>Ordine</th><th>Cliente</th><th>Piatti</th><th>Pagamento</th><th>Stato</th></tr>${os.length?os.slice().sort((a,b)=>a.slot.localeCompare(b.slot)).map(o=>`<tr><td class="num" style="font-weight:800;color:var(--accent)">${o.slot}</td><td class="num">#${o.n}</td><td><b>${esc(o.cust.name)}</b><div class="small muted">${o.mode==="consegna"?"Consegna · "+esc(o.addr):"Ritiro"}</div></td><td class="small">${o.rows.map(r=>{const p=products.find(x=>x.id===r.id);return r.q+"× "+(p?esc(p.n):"—")}).join(", ")}</td><td><span class="pill ${o.pay==="cash"?"unpaid":"pay"}">${o.pay==="cash"?"Contanti":PAYL[o.pay]+" · pagato"}</span></td><td><span class="pill">${{new:"Da confermare",prep:"Confermato",ready:"Pronto",done:o.mode==="consegna"?"Consegnato":"Ritirato"}[o.st]}</span></td></tr>`).join(""):`<tr><td colspan="6" class="muted">—</td></tr>`}</table></div>`;
 ...
 if(page==="orders"){
  ...
  const card=o=>`<div class="oc ${o.st}"><div class="h"><b>#${o.n} · ${esc(o.cust.name)}</b><span class="when num">${o.slot}</span></div>
   ...
   <div class="acts">${o.st==="new"?`<button class="btn sm ok" data-st="prep" data-n="${o.n}">Conferma</button><button class="btn sm danger" data-st="rej" data-n="${o.n}">Rifiuta</button>`:o.st==="prep"?`<button class="btn sm ok" data-st="ready" data-n="${o.n}">Segna pronto</button><button class="btn sm sec" data-print="${o.n}">Comanda</button>`:o.st==="ready"?`<button class="btn sm ok" data-st="done" data-n="${o.n}">${o.mode==="consegna"?"Consegnato":"Ritirato"}${o.pay==="cash"?" · incassa "+eur(o.tot.tot):""}</button>`:`<span class="small muted">Chiuso alle ${o.closed||"—"}</span>`}</div></div>`;
  const os=dayOrders(aday);
  M.innerHTML=`<div class="ph"><div><h2>Ordini</h2><p>Le prenotazioni si confermano in blocco alla chiusura, oppure una per una. Il cliente riceve un WhatsApp a ogni passaggio.</p></div><div style="display:flex;gap:8px"><button class="btn sm" id="confirmAll" ${os.some(o=>o.st==="new")?"":"disabled"}>Conferma tutti i nuovi</button><button class="btn sm sec" id="resetDemo">Ripristina esempi</button></div></div>
  ...
  bindDayTabs(M);
  M.querySelectorAll("[data-st]").forEach(b=>b.onclick=()=>{const o=orders.find(x=>x.n==b.dataset.n);const s=b.dataset.st;if(s==="rej"){if(!confirm(`Rifiutare la prenotazione #${o.n}? Il cliente riceve un messaggio e l'eventuale pagamento viene stornato.`))return;o.st="rej";toast(`#${o.n} rifiutata e cliente avvisato`)}else{o.st=s;if(s==="done")o.closed=nowHM();toast({prep:`#${o.n} confermata · entra in produzione`,ready:`#${o.n} pronta · cliente avvisato`,done:`#${o.n} chiusa`}[s])}persist();renderAdmin()});
  M.querySelectorAll("[data-print]").forEach(b=>b.onclick=()=>toast("Comanda inviata alla stampante di cucina"));
  $("#confirmAll").onclick=()=>{const n=os.filter(o=>o.st==="new");n.forEach(o=>o.st="prep");persist();renderAdmin();toast(`${n.length} prenotazioni confermate · WhatsApp inviati`)};
  $("#resetDemo").onclick=()=>{if(!confirm("Ripristinare menù, ordini e impostazioni di esempio?"))return;clearAll();location.reload()};
 }

 /* ---- Menù e giorni ---- */
 if(page==="menu"){
  M.innerHTML=`<div class="ph"><div><h2>Menù e giorni di produzione</h2><p>Per ogni piatto: in quali giorni si fa e quante porzioni al massimo. Il cliente vede solo ciò che quel giorno è in produzione. Quello che cambi qui si vede subito nel negozio.</p></div><button class="btn sm" id="addProd">+ Nuovo piatto</button></div>
  ...
  M.querySelectorAll("[data-limit]").forEach(i=>i.onchange=()=>{const p=products.find(x=>x.id==i.dataset.limit);p.limit=i.value===""?null:Math.max(0,+i.value);persist();toast(p.limit==null?"Nessun tetto: si cucina tutto il prenotato":`Tetto: ${p.limit} porzioni al giorno`)});
 ...
 if(page==="catering"){
  ...
  M.innerHTML=`<div class="ph"><div><h2>Richieste catering</h2><p>Arrivano dal modulo del sito. Il preventivo parte da qui con un link per l'acconto del 30%.</p></div></div>
  <div class="card" style="padding:0">${catReqs.length?catReqs.map(r=>{const f=FORMULE.find(x=>x.n===r.formula);const stima=f?f.pp*r.persone:0;return`<div class="cat-req"><div><b>${esc(r.nome)} · ${r.persone} persone · ${new Date(r.data+"T00:00").toLocaleDateString("it-IT",{day:"numeric",month:"long"})}</b><div class="m">${esc(r.tipo)} · ${esc(r.formula)} · ${esc(r.luogo||"luogo da definire")} · ${esc(r.tel)}${r.note?" · «"+esc(r.note)+"»":""}</div><div style="margin-top:6px;display:flex;gap:6px;align-items:center;flex-wrap:wrap"><span class="pill ${STL[r.st][1]}">${STL[r.st][0]}</span>${r.importo?`<span class="small num"><b>${eur(r.importo)}</b> · acconto ${eur(r.importo*0.3)}</span>`:`<span class="small muted num">stima listino ${eur(stima)}</span>`}${r.ex?'<span class="pill ex">esempio</span>':""}</div></div>
   <div style="display:flex;gap:6px;flex-direction:column">${r.st==="nuova"?`<button class="btn sm" data-q="${r.id}">Invia preventivo</button>`:r.st==="preventivo"?`<button class="btn sm ok" data-conf="${r.id}">Acconto ricevuto</button><button class="btn sm sec" data-lost="${r.id}">Non conclusa</button>`:""}</div></div>`}).join(""):'<div class="cat-req muted">Nessuna richiesta.</div>'}</div>`;
  M.querySelectorAll("[data-q]").forEach(b=>b.onclick=()=>{const r=catReqs.find(x=>x.id==b.dataset.q);const f=FORMULE.find(x=>x.n===r.formula);const v=prompt("Importo del preventivo in euro",String(f?f.pp*r.persone:0));if(v===null)return;r.importo=parseFloat(v.replace(",","."))||0;r.st="preventivo";persist();renderAdmin();toast("Preventivo inviato via email e WhatsApp con link per l'acconto")});
  M.querySelectorAll("[data-conf]").forEach(b=>b.onclick=()=>{const r=catReqs.find(x=>x.id==b.dataset.conf);r.st="confermata";persist();renderAdmin();toast("Evento confermato e messo in agenda")});
 ...
 if(page==="stats"){
  ...
  M.innerHTML=`<div class="ph"><div><h2>Incassi</h2><p>Solo prenotazioni online. Le vendite al banco restano sul registratore di cassa.</p></div></div>
  <div class="kpis"><div class="kpi"><div class="eyebrow">Incassato (ordini chiusi)</div><div class="v num">${eur(inc)}</div></div><div class="kpi"><div class="eyebrow">Già prenotato</div><div class="v num">${eur(booked)}<small>prossimi giorni</small></div></div><div class="kpi"><div class="eyebrow">Scontrino medio</div><div class="v num">${eur(all.length?all.reduce((s,o)=>s+o.tot.tot,0)/all.length:0)}</div></div><div class="kpi warn"><div class="eyebrow">Contanti ancora da incassare</div><div class="v num">${eur(cash)}</div></div></div>
  <div class="grid2"><div class="card"><h3>Piatti più prenotati</h3><ul class="top5">${top.map(([id,q])=>{const p=products.find(x=>x.id==id);return`<li><span>${p?esc(p.n):"—"}</span><b class="num">${q}</b></li>`}).join("")||"<li class='muted'>Ancora nessun ordine</li>"}</ul></div><div class="card"><h3>Come pagano</h3><ul class="top5">${Object.entries(PAYL).map(([k,l])=>`<li><span>${l}</span><b class="num">${all.filter(o=>o.pay===k).length}</b></li>`).join("")}</ul><p class="hint" style="margin:12px 0 0">Il contante è il rischio del modello sul venduto: in produzione ogni cliente ha un contatore di ordini non ritirati.</p></div></div>`;
 }

 /* ---- Impostazioni ---- */
 if(page==="settings"){
  M.innerHTML=`<div class="ph"><div><h2>Impostazioni</h2><p>Orari di chiusura prenotazioni, tempi, zone e pagamenti. Tutto si riflette nel negozio all'istante.</p></div></div>
  <div class="grid2">
   <div class="card"><h3>Chiusura prenotazioni</h3><div class="form"><label>Per il giorno dopo, entro le<input type="time" id="s_cut" value="${settings.cutoff}"></label><label>Per il giorno stesso, entro le<input type="time" id="s_scut" value="${settings.sameDayCutoff}" ${settings.sameDay?"":"disabled"}></label><label class="full"><span class="sw ${settings.sameDay?"on":""}" id="s_same"><i></i>Accetta ordini per il giorno stesso (solo piatti già in produzione)</span></label></div>
   ...
   <div class="card"><h3>Orari di apertura</h3><div class="hours">${Object.keys(DN).map(k=>{const h=settings.hours[k];return`<div class="row"><b>${DN[k]}</b>${h?`<span class="num">${h[0].join("–")}</span><span class="num">${h[1]?h[1].join("–"):""}</span>`:`<span class="muted">Chiuso</span><span></span>`}</div>`}).join("")}</div><p class="hint" style="margin:12px 0 0">In questa versione gli orari sono fissi; nel sito finito si modificano qui, con chiusure straordinarie e ferie.</p>
    <h3 style="margin-top:20px">Metodi di pagamento</h3><div style="display:grid;gap:10px">${[["card","Carta online (Stripe)"],["satispay","Satispay"],["cash","Contanti al ritiro / alla consegna"]].map(([k,l])=>`<label class="sw ${settings.pay[k]?"on":""}" data-pay="${k}"><i></i>${l}</label>`).join("")}</div></div>
   <div class="card zones" style="grid-column:1/-1"><h3>Zone di consegna</h3><div class="row small muted"><span>Comune</span><span>Costo consegna</span><span>Ordine minimo</span><span></span></div>${settings.zones.map((z,i)=>`<div class="row"><input value="${esc(z.c)}" data-zc="${i}"><input class="num" type="number" step="0.5" value="${z.cost}" data-zcost="${i}"><input class="num" type="number" step="1" value="${z.min}" data-zmin="${i}"><button class="btn sm sec" data-zdel="${i}" ${i===0?"disabled title='La prima zona è la sede'":""}>Togli</button></div>`).join("")}<button class="btn sm ghost" id="zadd" style="margin-top:6px">+ Aggiungi comune</button></div>
  </div>`;
  $("#s_cut").onchange=e=>{settings.cutoff=e.target.value||"20:00";persist();toast("Chiusura prenotazioni aggiornata")};
  ...
  $("#s_cap").onchange=e=>{settings.cap=+e.target.value||6;persist();toast("Capacità per fascia aggiornata")};
