...
function renderAdmin(){
 ...
 if(page==="catering"){
  ...
  <div class="card" style="padding:0">${catReqs.length?catReqs.map(r=>{const f=FORMULE.find(x=>x.n===r.formula);const stima=f?f.pp*r.persone:0;return`<div class="cat-req"><div><b>${esc(r.nome)} · ${r.persone} persone · ${new Date(r.data+"T00:00").toLocaleDateString("it-IT",{day:"numeric",month:"long"})}</b><div class="m">${[r.tipo,r.formula,r.luogo,r.tel].filter(Boolean).map(esc).join(" · ")}${r.note?" · «"+esc(r.note)+"»":""}</div><div style="margin-top:6px;display:flex;gap:6px;align-items:center;flex-wrap:wrap"><span class="pill ${STL[r.st][1]}">${STL[r.st][0]}</span>${r.importo?`<span class="small num"><b>${eur(r.importo)}</b> · acconto ${eur(r.importo*0.3)}</span>`:`<span class="small muted num">stima listino ${eur(stima)}</span>`}${r.ex?'<span class="pill ex">esempio</span>':""}</div></div>
