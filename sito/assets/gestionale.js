...
function renderAdmin(){
 ...
 if(page==="orders"){
  ...
  bindDayTabs(M);
  M.querySelectorAll("[data-st]").forEach(b=>b.onclick=()=>{const o=orders.find(x=>x.n==b.dataset.n);const s=b.dataset.st;if(s==="rej"){if(!confirm(`Rifiutare l'ordine #${o.n}? Il cliente riceve un messaggio e, se ha già pagato, va rimborsato.`))return;o.st="rej";toast(`#${o.n} rifiutato, cliente avvisato`)}else{o.st=s;if(s==="done")o.closed=nowHM();toast({prep:`#${o.n} confermato, va in produzione`,ready:`#${o.n} pronto, cliente avvisato`,done:`#${o.n} chiuso`}[s])}persist();renderAdmin()});
