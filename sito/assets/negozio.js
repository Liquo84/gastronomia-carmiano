...
/* ---------- disegno ---------- */
function renderHero(){
 const open=isOpenNow();const h=hoursOf(TODAY);
 $("#chips").innerHTML=`<span class="chip"><i class="st ${open?"":"closed"}"></i>${open?"Bottega aperta adesso":(h?"Bottega chiusa ora":"Oggi chiusa")}</span><span class="chip">Cucina salentina</span><span class="chip">Si cucina solo il prenotato</span>`;
 $("#infoCut").textContent=`ordina entro le ${settings.cutoff} di oggi`;
 $("#infoToday").textContent=settings.sameDay?`fino alle ${settings.sameDayCutoff}, solo piatti già in produzione`:"non disponibile: solo su prenotazione";
 const minCost=Math.min(...settings.zones.map(z=>z.cost));const others=settings.zones.filter(z=>z.cost>0);
 $("#infoDeliv").textContent=`${settings.zones[0].cost===0?"gratis a "+settings.zones[0].c:"da "+eur(minCost)}${others.length?", da "+eur(Math.min(...others.map(z=>z.cost)))+" nei comuni vicini":""}`;
 $("#pauseBanner").classList.toggle("hide",!settings.pause);
}
function renderDays(){
 const ds=nextDays(7);
 $("#days").innerHTML=ds.map(k=>{const st=orderState(k);const cnt=k===day?"on":"";const d=new Date(k+"T00:00");return`<button class="day ${cnt} ${st.open?"":"off"}" data-day="${k}" ${st.open?"":"disabled"}><small>${DN[wd(k)].slice(0,3)} ${d.getDate()}</small><b>${dlabel(k)}</b><span>${st.open?(k===day?"✓ stai ordinando":"prenotazioni "+st.why):st.why}</span></button>`}).join("");
 ...
 $("#dayCut").textContent=st.open?(day===TODAY?`Per oggi si ordina fino alle ${settings.sameDayCutoff}, sui piatti già in produzione`:day===TOMORROW?`Prenotazioni aperte fino alle ${settings.cutoff} di stasera`:`Prenotazioni aperte fino alle ${settings.cutoff} del giorno prima`):"Prenotazioni chiuse per questo giorno";
}
...
function renderMenu(){
 const st=orderState(day);
 const html=CATS.map((c,i)=>{const list=products.filter(p=>p.cat===c&&offeredOn(p,day));if(!list.length)return"";return`<div id="cat-${i}"><div class="cat-h"><h2>${c}</h2><span>${list.length} ${list.length===1?"piatto":"piatti"} ${dlabel(day).toLowerCase()}</span></div>${list.map(p=>{const q=cart[p.id]||0;const rem=remaining(p,day);const out=rem<=0;const dis=out||!st.open;const few=rem!==Infinity&&rem<=5&&!out;return`<div class="item ${out?"off":""}"><div><div class="n">${esc(p.n)}</div>${p.d?`<div class="d">${esc(p.d)}</div>`:""}<div class="meta"><span class="price num">${eur(p.p)}</span>${p.veg?'<span class="tag veg">Veg</span>':""}${p.hot?'<span class="tag hot">Più prenotato</span>':""}${out?'<span class="tag out">Esaurito per questo giorno</span>':few?`<span class="tag few">Ne restano ${rem}</span>`:""}</div></div><div class="act">${q>0&&!dis?`<span class="stepper"><button data-dec="${p.id}" aria-label="Togli">−</button><b class="num">${q}</b><button data-inc="${p.id}" aria-label="Aggiungi">+</button></span>`:`<button class="add" data-inc="${p.id}" ${dis?"disabled":""} aria-label="Aggiungi ${esc(p.n)}">+</button>`}</div></div>`}).join("")}</div>`}).join("");
...
function renderCart(){
 ...
 <span class="hint">Scegli l'ora al passo successivo. Paghi alla fine, anche in contanti al ritiro.</span></div>`
 :`<div class="cart-empty">Il carrello è vuoto.<br>Scegli dal menù di ${dlabel(day).toLowerCase()}.</div>`;
...
function renderFormule(){
 $("#formule").innerHTML=FORMULE.map(f=>`<div class="formula"><h3>${f.n}</h3><span class="pp">da ${eur(f.pp)} a persona</span><ul>${f.l.map(x=>`<li>${x}</li>`).join("")}</ul></div>`).join("");
...
function renderCheckout(){
 ...
 if(co.step===1){const sl=slotsFor(day);body=`<div class="opts">
  <label class="opt ${co.mode==="ritiro"?"on":""}" data-m="ritiro"><span class="rad"></span><span><b>Ritiro in bottega</b><span>Via Leverano 29/A · lo trovi pronto all'ora scelta</span></span><span class="num">gratis</span></label>
  <label class="opt ${co.mode==="consegna"?"on":""}" data-m="consegna"><span class="rad"></span><span><b>Consegna a domicilio</b><span>Carmiano e comuni vicini · nella fascia scelta</span></span><span class="num">da ${eur(Math.min(...settings.zones.map(z=>z.cost)))}</span></label></div>
  ${co.mode==="consegna"?`<div style="margin:14px 0 0"><label class="small muted" style="font-weight:700">Comune di consegna<select id="zoneSel">${settings.zones.map(z=>`<option ${z.c===co.zone?"selected":""}>${z.c} · ${z.cost?eur(z.cost):"gratis"} · min. ${eur(z.min)}</option>`).join("")}</select></label></div>`:""}
  <div class="eyebrow" style="margin:22px 0 4px">A che ora, ${dlabel(day).toLowerCase()}</div>
  <span class="hint">Fasce di 15 minuti negli orari della bottega. Le fasce piene sono spente.</span>
  <div class="slots">${sl.length?sl.map(s=>`<button data-slot="${s.s}" class="${co.slot===s.s?"on":""}" ${s.full?"disabled title='Fascia piena'":""}>${s.s}</button>`).join(""):'<span class="muted small">Non ci sono più fasce disponibili per questo giorno.</span>'}</div>
  ${co.err?`<p class="err">${co.err}</p>`:""}`;
 }else if(co.step===2){body=`<div class="form">
  <label>Nome e cognome<input id="f_name" value="${esc(co.name)}" placeholder="Come ti chiamiamo al ritiro"></label>
  <label>Telefono<input id="f_tel" type="tel" value="${esc(co.tel)}" placeholder="Ti scriviamo su WhatsApp"></label>
  <label class="full">Email <span class="muted" style="font-weight:500">(facoltativa, per la ricevuta)</span><input id="f_email" type="email" value="${esc(co.email)}"></label>
  ${co.mode==="consegna"?`<label class="full">Indirizzo e numero civico · ${co.zone}<input id="f_addr" value="${esc(co.addr)}" placeholder="Via, numero"></label><label>Citofono / scala<input id="f_cit" value="${esc(co.citofono)}"></label>`:""}
  <label class="full">Note per la cucina<textarea id="f_note" rows="2" placeholder="Senza cipolla, porzione doppia di pane…">${esc(co.note)}</textarea></label></div>${co.err?`<p class="err">${co.err}</p>`:""}`;
 }else if(co.step===3){const P=[["card","Carta online","Visa, Mastercard, Apple Pay, Google Pay. Incassiamo ora, niente da fare al ritiro.",settings.pay.card],["satispay","Satispay","Confermi dall'app, addebito immediato.",settings.pay.satispay],["cash",co.mode==="consegna"?"Contanti alla consegna":"Contanti al ritiro","Paghi in bottega. Cuciniamo apposta per te: se non passi, avvisaci.",settings.pay.cash]].filter(x=>x[3]);
  ...
  ${co.pay==="card"?`<div class="form" style="margin-top:16px"><label class="full">Numero carta<input value="4242 4242 4242 4242" readonly></label><label>Scadenza<input value="12/28" readonly></label><label>CVC<input value="123" readonly></label><span class="full hint">In questa versione di prova non viene addebitato nulla. Nel sito finito questo riquadro è il modulo sicuro di Stripe: i dati della carta non passano mai dal nostro sito.</span></div>`:""}`;
  ...
  <div class="row muted"><span>Pagamento</span><span>${{card:"Carta online",satispay:"Satispay",cash:"Contanti"}[co.pay]}</span></div>
  <span class="hint">Confermando prenoti le porzioni: le cuciniamo apposta per te. Puoi annullare gratis fino alla chiusura delle prenotazioni; dopo, l'eventuale pagamento non è rimborsabile.</span></div>`;
  ...
  <p class="muted" style="margin:0 0 16px">${ord.st==="rej"?"La bottega non può accettare questa prenotazione: ti ha scritto su WhatsApp al "+esc(ord.cust.tel)+"."+(ord.pay!=="cash"?" Il pagamento viene stornato.":""):`${dfull(ord.date)} alle ${ord.slot}, ${ord.mode==="consegna"?"a "+esc(ord.addr):"in Via Leverano 29/A"}.<br>Ti avvisiamo su WhatsApp al ${esc(ord.cust.tel)} a ogni passaggio.`}</p>
  <div class="timeline">${[["Ricevuta","La bottega la vede adesso"],["Confermata",ord.date===TODAY?"È già in produzione":"Entra nella produzione di "+dlabel(ord.date).toLowerCase()],["Pronta",ord.mode==="consegna"?"Il fattorino sta partendo":"Puoi passare a ritirarla"],[ord.mode==="consegna"?"Consegnata":"Ritirata","Buon appetito"]].map((t,i)=>`<div class="tl ${i<idx?"done":i===idx?"now":""}"><i class="b"></i><div><b>${t[0]}</b><span>${t[1]}</span></div></div>`).join("")}</div>
  <p class="hint" style="margin-top:14px">Questa pagina si aggiorna da sola a ogni passaggio.</p></div>`;
 }
 const canBack=co.step>1&&co.step<5;
 o.innerHTML=`<div class="modal" role="dialog" aria-label="Checkout"><header><div><h2>${titles[co.step]}</h2>${co.step<5?`<small>Prenotazione per ${dfull(day)}</small>`:""}</div><button class="x" id="coX" aria-label="Chiudi">×</button></header><div class="mbody">${co.step<5?`<div class="steps">${[1,2,3,4].map(i=>`<div class="${i<=co.step?"done":""}"></div>`).join("")}</div>`:""}${body}</div>
 ...
 const nx=$("#coNext");if(nx)nx.onclick=()=>{
  ...
  if(co.step===2){co.name=$("#f_name").value.trim();co.tel=$("#f_tel").value.trim();co.email=$("#f_email").value.trim();co.note=$("#f_note").value.trim();if(co.mode==="consegna"){co.addr=$("#f_addr").value.trim();co.citofono=$("#f_cit").value.trim()}
   if(!co.name||co.tel.replace(/\D/g,"").length<9||(co.mode==="consegna"&&!co.addr)){co.err="Servono nome, un numero di telefono valido"+(co.mode==="consegna"?" e l'indirizzo di consegna.":".");return renderCheckout()}co.step=3;co.err="";return renderCheckout()}
